...
from utils.fmp_utils import get_us_stock_financials, fmp_cached
...
async def company_financial_info(market: str, symbol: str, task: str, trigger_time: str=None) -> str:
    tools_config = ToolManagerConfig(tool_paths=[
        "tools.corp_info_fmp.company_income",
        "tools.corp_info_fmp.company_balance_sheet",
        "tools.corp_info_fmp.company_cash_flow",
        "tools.corp_info_fmp.company_forecast",
        "tools.corp_info_fmp.company_express",
        "tools.corp_info_fmp.company_dividend",
    ])
...
class CompanyIncomeInput(BaseModel):
    ...
    period: str = Field(description="The period of the financial data. Format: YYYYMMDD. \
                        ...
                        for third quarter report). US-Stock: use 'annual' or 'quarter'.")
    trigger_time: str = Field(description="The trigger time of the financial data. Format: YYYY-MM-DD HH:MM:SS.")
@tool(
...
async def company_income(market: str, symbol: str, period: str, trigger_time: str=None) -> str:
    if market == "CN-Stock":
        ...
        df = get_us_stock_financials(symbol=symbol, statement_type="income", period=period_type, verbose=False)
        if df.empty:
            return {"error": f"No income data found for {symbol}"}
        return df.to_markdown()
    else:
...
async def company_balance_sheet(market: str, symbol: str, period: str, trigger_time: str=None) -> str:
    if market == "CN-Stock":
        ...
        df = get_us_stock_financials(symbol=symbol, statement_type="balance", period=period_type, verbose=False)
        if df.empty:
...
async def company_cash_flow(market: str, symbol: str, period: str, trigger_time: str=None) -> str:
    if market == "CN-Stock":
        ...
        df = get_us_stock_financials(symbol=symbol, statement_type="cash", period=period_type, verbose=False)
        if df.empty:
...
if __name__ == "__main__":
    result = asyncio.run(company_financial_info.ainvoke({"market": "US-Stock", "symbol": "AAPL", "task": "最近财报", "trigger_time": "2025-01-09 15:00:00"}))
    print(result)
    pass