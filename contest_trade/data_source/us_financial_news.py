...
class USFinancialNews(DataSourceBase):
    def __init__(self):
        super().__init__("us_financial_news")
    ...
    async def get_data(self, trigger_time: str) -> pd.DataFrame:
        try:
            ...
            # 从FMP获取美股新闻
            news_df = get_us_stock_news(limit=30, verbose=False)
            ...
            for _, row in news_df.iterrows():
                # 处理发布时间
                pub_time = row.get('publishedDate', trigger_time)
                ...
                content = str(row.get('text', ''))[:2000]  # 限制内容长度
                url = str(row.get('url', ''))
...
if __name__ == "__main__":
    us_news = USFinancialNews()
    print(asyncio.run(us_news.get_data("2024-12-01 10:00:00")))
